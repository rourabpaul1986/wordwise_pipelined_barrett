...
def barrett_multiplication_wordwise(a, b, n, l, w, f,  e0, e1, e2, e12, faults, p, mode, injection, verbose):
    """
    ...
    for i in range(num_chunks):
        ...
        for j in range(num_chunks):
            ...
            if p %2==1 and mode[2]=="r":
                 if(injection=="r"):
             	        r=flip_random_bits(r, k, f)
                 elif(injection=="b"):
                         r=flip_burst_bits(r, k, f)
            #elif mode[2]!="r":
            #    sys.exit(f"{mode[2]} is wrong fault mode, command should be -m xxr")  
            elif mode[2]=="x":
                 r=r    
...
def CT_Butterfly(i,j,n,x_out, A, twiddle_factor, M, length, f, l, w, array,  e0, e1, e2,e12, faults, p, mode, injection, verbose):
    ...
    for k in range(halflen):
        ...
        #p=p+1
        v, e0, e1, e2, e12, p = barrett_multiplication_wordwise(A[k + halflen], twiddle_factor, M, l, w, f,  e0, e1, e2, e12, faults, p, mode, injection, verbose)
        x_out[k] = (u + v) % M
...
class fNTT:
    ...
    def forward(self, x_in, N, f, l, w,  e0, e1, e2, e12, faults, p, mode, injection, verbose=False):
        ...
        for i in range(self.Nlen):
            ...
            for j in range(n):
                ...
                array,  e0, e1, e2,e12, p=CT_Butterfly(i,j,n,x[seqlen * j: seqlen * (j + 1)], x[seqlen * j: seqlen * (j + 1)], twiddle_factor, self.M, seqlen, f, l, w, array,  e0, e1, e2,e12, faults, p, mode, injection, verbose)
              
        #print(f"array: {array}")
        #print("--------------------")    
        return bit_reverse([i % self.M for i in x], self.Nlen),  e0, e1, e2,e12, p

    def inverse(self, x_in, N, f, l, w, e0, e1, e2, e12, faults, p, mode, injection, verbose=False):
        ...
        return bit_reverse(x, self.Nlen),  e0, e1, e2,e12, p
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--fault", type=int, default=0, required=False,
                        help="Number of Faulty bits (integer value)")
    parser.add_argument("-fl", "--faulty_loop", type=int, default=total_loop, required=False,
                        help="number of NTT loops affected by fault ")
    parser.add_argument("-i", "--injection", type=str, required=False,
                        help="fault injection type: random/burts")
    parser.add_argument("-m", "--mode", type=str,  default="cqr", required=False,
    ...
    w = 6  # how many binary bits will be processed
    seed="abcd"
    ...
    i, j = 1, 2
    seed = rho + bytes([i, j])
    A = kyber_xof_12bit(seed, N, M)
    faults = 0
    p=0

    p=flip_random_bits(p, total_loop , fl)
    ntt_A, e0, e1, e2, e12, p = ntt.forward(A, N, f, l, w,  e0, e1, e2, e12, faults, p,  mode, injection, verbose=verbose)
    #print(f"p: {p}")
    ...
    e2 = 0
    e12 = 0
    inv_ntt_A,  e0, e1, e2,e12, p = ntt.inverse(ntt_A,N,f, l, w, e0, e1, e2,e12, faults, p, mode, injection, verbose=verbose)
    #print(f"Inverse NTT A: {inv_ntt_A}")
    check_arrays(inv_ntt_A, A)
    print(f"RESULT {e0_val} {e1_val} {e2_val} {e12_val}")
