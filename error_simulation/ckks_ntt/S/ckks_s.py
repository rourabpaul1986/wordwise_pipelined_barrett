...
def is_primitive(alpha, N, M):
    return pow(alpha, 2 * N, M) == 1 and pow(alpha, N, M) != 1

def find_primitive(N, M):
    required = 2 * N
    for g in range(2, M):
        if pow(g, required, M) == 1 and pow(g, required // 2, M) != 1:
            return g
    return 0
...
class fNTT:
    def __init__(self, N, M, alpha=None):
        ...
        self.Ninv = pow(self.N, self.M - 2, self.M)
        print(M)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-fl", "--faulty_loop", type=int, default=4096, required=False,
    ...
    parser.add_argument("-m", "--mode", type=str,  default="xxx", required=False,
    ...
    parser.add_argument("-l", "--length", type=int, default=32, required=False,
                        help="Number of partitions (integer value)")
    parser.add_argument("-w", "--wordsize", type=int, default=8, required=False,
                        help="Number of partitions (integer value)")
    parser.add_argument("-N", "--degree", type=int, default=4096, required=False,
                        help="Number of partitions (integer value)")
    parser.add_argument("-M", "--mod", type=int, default=1811939329, required=False,
                        help="Number of partitions (integer value)")
    ...
    g = 31
    alpha = pow(g, (M - 1) // (2 * N), M)
    ntt = fNTT(N, M, alpha=alpha)
    ...
    #N = 4096
    #q_list = [2**40, 2**40, 2**40]
    #A = kyber_xof_12bit(seed, N, M)
    if (M - 1) % (2 * N) != 0:
     print(f"Invalid modulus: M={M} is not suitable for NTT with N={N}")
     sys.exit(1)
    A = generate_sparse_s(N, M)
    faults = 0
    p=0
    q_fault=0
    r_fault=0
